...
if ( $type === 'artist' ) {
	...
	$title = __( 'Artists', 'kontentainment-lists' );
} elseif ( $type === 'track' ) {
	...
	" );
	$total = $wpdb->get_var( "SELECT COUNT(*) FROM {$table} {$where}" );
	$title = __( 'Tracks', 'kontentainment-lists' );
} else {
	...
	$title = __( 'Entities (Advanced Explorer)', 'kontentainment-lists' );
}
...
<div class="charts-admin-wrap">
	<header class="charts-header">
		<div>
			...
			<p class="subtitle"><?php printf( _n( '%s record found', '%s records found', $total, 'kontentainment-lists' ), number_format( $total ) ); ?></p>
		</div>
		<div class="charts-actions">
			<form method="get" action="">
				<input type="hidden" name="page" value="<?php echo esc_attr( $page ); ?>">
				<input type="text" name="s" value="<?php echo esc_attr( $search ); ?>" placeholder="<?php _e( 'Search...', 'kontentainment-lists' ); ?>" class="charts-search-input">
			</form>
		</div>
	</header>
		...
		<div class="charts-card" style="grid-column: span 12; padding: 0; overflow: hidden;">
			<?php if ( empty( $items ) ) : ?>
				<div style="padding: 60px; text-align: center; color: #6b7280;">
					...
					<h3 style="margin-top: 20px;"><?php _e( 'No records matching criteria', 'kontentainment-lists' ); ?></h3>
				</div>
			<?php else : ?>
				<table class="charts-table">
					<thead>
						<tr>
							<th style="padding-left: 24px;"><?php _e( 'Title / Name', 'kontentainment-lists' ); ?></th>
							<?php if ( $type === 'track' || $type === 'advanced' ) : ?>
								<th><?php _e( 'Artist', 'kontentainment-lists' ); ?></th>
							<?php endif; ?>
							<?php if ( $type === 'advanced' ) : ?>
								<th><?php _e( 'Best Rank', 'kontentainment-lists' ); ?></th>
								<th><?php _e( 'Longevity', 'kontentainment-lists' ); ?></th>
							<?php else : ?>
								<th><?php _e( 'Slug', 'kontentainment-lists' ); ?></th>
								<th><?php _e( 'Spotify ID', 'kontentainment-lists' ); ?></th>
							<?php endif; ?>
								...
								<td style="text-align: right; padding-right: 24px;">
									<div style="display: flex; gap: 5px; justify-content: flex-end;">
										<?php if ( ! empty( $item->slug ) ) : ?>
											...
											$view_url = home_url( '/lists/' . $slug_path . '/' . $item->slug );
											?>
											<a href="<?php echo esc_url( $view_url ); ?>" target="_blank" class="charts-badge charts-badge-neutral" style="text-decoration: none;"><?php _e( 'View', 'kontentainment-lists' ); ?></a>
										<?php endif; ?>
										...
										<?php if ( $type !== 'advanced' ) : ?>
											<form method="post" style="display: inline;" onsubmit="return confirm('Really delete entity? Data in entries will NOT be deleted, but relationships will break.');">
												...
												<button type="submit" class="charts-badge charts-badge-danger" style="border: none; cursor: pointer;"><?php _e( 'Delete', 'kontentainment-lists' ); ?></button>
											</form>
										<?php endif; ?>
									</div>
								</td>
