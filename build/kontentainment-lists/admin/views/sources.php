...
<div class="charts-admin-wrap">
	<header class="charts-header">
		<div>
			<h1><?php echo $action === 'edit' ? __( 'Edit Source', 'kontentainment-lists' ) : ( $action === 'add' ? __( 'Add New Source', 'kontentainment-lists' ) : __( 'Data Sources', 'kontentainment-lists' ) ); ?></h1>
			<p class="subtitle"><?php _e( 'Manage Spotify & YouTube chart sources', 'kontentainment-lists' ); ?></p>
		</div>
		<div class="charts-actions">
			<?php if ( $action === 'list' ) : ?>
				<a href="<?php echo admin_url( 'admin.php?page=charts-sources&action=add' ); ?>" class="charts-btn charts-btn-primary"><?php _e( 'Add New Source', 'kontentainment-lists' ); ?></a>
			<?php else : ?>
				<a href="<?php echo admin_url( 'admin.php?page=charts-sources' ); ?>" class="charts-btn charts-btn-secondary"><?php _e( 'Back to List', 'kontentainment-lists' ); ?></a>
			<?php endif; ?>
		</div>
	</header>
		...
		<?php if ( $action === 'list' ) : ?>
			<div class="charts-card" style="grid-column: span 12; padding: 0; position: relative; overflow: hidden;">
				<table class="charts-table">
					<thead>
						<tr>
							<th style="padding-left: 24px;"><?php _e( 'Source Name', 'kontentainment-lists' ); ?></th>
							<th><?php _e( 'Platform', 'kontentainment-lists' ); ?></th>
							<th><?php _e( 'Type', 'kontentainment-lists' ); ?></th>
							<th><?php _e( 'Country', 'kontentainment-lists' ); ?></th>
							<th><?php _e( 'Frequency', 'kontentainment-lists' ); ?></th>
							<th><?php _e( 'Status', 'kontentainment-lists' ); ?></th>
							<th><?php _e( 'Last Run', 'kontentainment-lists' ); ?></th>
							<th style="text-align: right; padding-right: 24px;"><?php _e( 'Actions', 'kontentainment-lists' ); ?></th>
						</tr>
					</thead>
					<tbody>
						<?php if ( empty( $sources ) ) : ?>
							<tr>
								<td colspan="8" style="text-align: center; color: var(--charts-gray-500); padding: 40px;">
									<div style="font-size: 14px; margin-bottom: 10px;"><?php _e( 'No sources found in the registry.', 'kontentainment-lists' ); ?></div>
									<button class="charts-btn charts-btn-secondary" onclick="location.reload();"><?php _e( 'Refresh registry', 'kontentainment-lists' ); ?></button>
								</td>
							</tr>
						<?php else : ?>
								...
								<tr data-source-id="<?php echo esc_attr( $source->id ); ?>">
									<td style="padding-left: 24px;">
										...
										<div style="font-size: 11px; font-weight: 500; color: var(--charts-gray-500); margin-top: 2px;">
											<?php echo $source->source_type === 'manual_import' ? '<em>' . esc_html__('Manual CSV Import', 'kontentainment-lists') . '</em>' : esc_html( $source->source_url ); ?>
										</div>
									</td>
									<td>
									...
									<td style="text-align: right; padding-right: 24px;">
										<div style="display: flex; gap: 8px; justify-content: flex-end;">
											<?php if ( $source->source_type === 'live_scrape' ) : ?>
												<button class="charts-btn charts-btn-primary handle-run-import" style="padding: 6px 12px; font-size: 11px;"><?php _e( 'Fetch Now', 'kontentainment-lists' ); ?></button>
											<?php endif; ?>
											<a href="<?php echo admin_url( 'admin.php?page=charts-sources&action=edit&id=' . $source->id ); ?>" class="charts-btn charts-btn-secondary" style="padding: 6px 12px; font-size: 11px; text-decoration: none;"><?php _e( 'Edit', 'kontentainment-lists' ); ?></a>
											<form method="post" action="" onsubmit="return confirm('Delete this source?');" style="display:inline;">
												...
												<button type="submit" class="charts-btn" style="padding: 6px 12px; font-size: 11px; color: red; border-color: rgba(255,0,0,0.1); background: transparent;"><?php _e( 'Delete', 'kontentainment-lists' ); ?></button>
											</form>
										</div>
									</td>
								</tr>
			...
			<div class="charts-card" style="grid-column: span 12;">
				<form method="post" action="<?php echo admin_url('admin.php?page=charts-sources'); ?>">
					...
					<table class="form-table">
						<tr>
							<th scope="row"><label for="source_name"><?php _e( 'Source Name', 'kontentainment-lists' ); ?></label></th>
						...
						<tr>
							<th scope="row"><label for="platform"><?php _e( 'Platform', 'kontentainment-lists' ); ?></label></th>
							<td>
						...
						<tr>
							<th scope="row"><label for="source_type"><?php _e( 'Source Type', 'kontentainment-lists' ); ?></label></th>
							<td>
						...
						<tr>
							<th scope="row"><label for="source_url"><?php _e( 'Source URL', 'kontentainment-lists' ); ?></label></th>
							<td>
								...
								<p class="description"><?php _e( 'Required for Live Scrape sources. For Spotify manual imports, this can be "manual".', 'kontentainment-lists' ); ?></p>
							</td>
						</tr>
						<tr>
							<th scope="row"><label for="country_code"><?php _e( 'Country Code', 'kontentainment-lists' ); ?></label></th>
						...
						<tr>
							<th scope="row"><label for="frequency"><?php _e( 'Frequency', 'kontentainment-lists' ); ?></label></th>
							<td>
						...
						<tr>
							<th scope="row"><label for="chart_type"><?php _e( 'List Type', 'kontentainment-lists' ); ?></label></th>
							<td>
						...
						<tr>
							<th scope="row"><label for="parser_key"><?php _e( 'Parser Key', 'kontentainment-lists' ); ?></label></th>
						...
						<tr>
							<th scope="row"><label for="is_active"><?php _e( 'Is Active?', 'kontentainment-lists' ); ?></label></th>
							<td><input type="checkbox" name="is_active" id="is_active" value="1" <?php checked( $edit_source ? $edit_source->is_active : 1 ); ?>></td>
						</tr>
					</table>

					<p class="submit">
						<button type="submit" class="button button-primary button-large"><?php _e( 'Save Source', 'kontentainment-lists' ); ?></button>
						<a href="<?php echo admin_url( 'admin.php?page=charts-sources' ); ?>" class="button button-secondary button-large"><?php _e( 'Cancel', 'kontentainment-lists' ); ?></a>
					</p>
				</form>
			</div>
		<?php endif; ?>
