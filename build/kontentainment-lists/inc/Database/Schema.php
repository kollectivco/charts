...
class Schema {
	...
	public function install() {
		...
		$queries = array(
			...
			// 15. Editorial analytics
			"CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}charts_page_metrics` (
				`id` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT,
				`event_date` DATE NOT NULL,
				`event_type` VARCHAR(20) NOT NULL DEFAULT 'view',
				`page_type` VARCHAR(30) NOT NULL DEFAULT 'list',
				`object_type` VARCHAR(30) NOT NULL DEFAULT 'list',
				`object_id` BIGINT(20) UNSIGNED NOT NULL DEFAULT 0,
				`object_slug` VARCHAR(191) DEFAULT NULL,
				`total_count` BIGINT(20) UNSIGNED NOT NULL DEFAULT 0,
				`updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
				PRIMARY KEY (`id`),
				UNIQUE KEY `metric_unique` (`event_date`,`event_type`,`page_type`,`object_type`,`object_id`,`object_slug`),
				KEY `event_lookup` (`event_type`,`page_type`),
	...
	private function run_column_upgrades( $wpdb ) {
		...
		$needed = array(
			...
			'country_code'  => "VARCHAR(10) DEFAULT NULL",
			'territory_code' => "VARCHAR(10) DEFAULT NULL",
			'chart_type'    => "VARCHAR(50) DEFAULT NULL",
			'album_title'   => "VARCHAR(255) DEFAULT NULL",
			'source_item_id' => "VARCHAR(100) DEFAULT NULL",
			'source_item_key' => "VARCHAR(191) DEFAULT NULL",
		...
		foreach ( array( $wpdb->prefix . 'charts_tracks', $wpdb->prefix . 'charts_videos' ) as $tbl ) {
			...
			if ( $tbl === $wpdb->prefix . 'charts_tracks' && ! in_array( 'soundcharts_track_id', $cols, true ) ) {
				$wpdb->query( "ALTER TABLE `$tbl` ADD COLUMN `soundcharts_track_id` VARCHAR(100) DEFAULT NULL" );
			}
		}
		$artists_tbl = $wpdb->prefix . 'charts_artists';
		$artist_cols = $wpdb->get_col( "DESCRIBE $artists_tbl", 0 );
		if ( ! in_array( 'soundcharts_artist_id', $artist_cols, true ) ) {
			$wpdb->query( "ALTER TABLE `$artists_tbl` ADD COLUMN `soundcharts_artist_id` VARCHAR(100) DEFAULT NULL" );
		}
		$albums_tbl = $wpdb->prefix . 'charts_albums';
		$album_cols = $wpdb->get_col( "DESCRIBE $albums_tbl", 0 );
		if ( ! in_array( 'soundcharts_album_id', $album_cols, true ) ) {
			$wpdb->query( "ALTER TABLE `$albums_tbl` ADD COLUMN `soundcharts_album_id` VARCHAR(100) DEFAULT NULL" );
		}
		...
		$runs_tbl = $wpdb->prefix . 'charts_import_runs';
		$run_cols = $wpdb->get_col( "DESCRIBE $runs_tbl", 0 );
		$run_needed = array(
			'import_source'      => "VARCHAR(50) DEFAULT NULL",
			'chart_definition_id' => "BIGINT(20) UNSIGNED DEFAULT NULL",
			'period_id'          => "BIGINT(20) UNSIGNED DEFAULT NULL",
			'batch_id'           => "BIGINT(20) UNSIGNED DEFAULT NULL",
			'updated_entries'    => "INT(11) DEFAULT 0",
			'created_artists'    => "INT(11) DEFAULT 0",
			'created_tracks'     => "INT(11) DEFAULT 0",
			'skipped_duplicates' => "INT(11) DEFAULT 0",
			'error_count'        => "INT(11) DEFAULT 0",
			'request_count'      => "INT(11) DEFAULT 0",
			'context_json'       => "LONGTEXT DEFAULT NULL",
		);
		foreach ( $run_needed as $col => $definition ) {
			if ( ! in_array( $col, $run_cols, true ) ) {
				$wpdb->query( "ALTER TABLE `$runs_tbl` ADD COLUMN `$col` $definition" );
			}
		}

		$intel_tbl = $wpdb->prefix . 'charts_intelligence';
		$intel_cols = $wpdb->get_col( "DESCRIBE $intel_tbl", 0 );
		$intel_needed = array(
			'trend_type'            => "VARCHAR(30) DEFAULT 'stable'",
			'velocity_score'        => "DECIMAL(10,2) DEFAULT 0",
			'predicted_rank'        => "INT(11) DEFAULT NULL",
			'prediction_confidence' => "DECIMAL(5,2) DEFAULT 0",
			'current_rank'          => "INT(11) DEFAULT NULL",
			'previous_rank'         => "INT(11) DEFAULT NULL",
			'chart_history_json'    => "LONGTEXT DEFAULT NULL",
		);
		foreach ( $intel_needed as $col => $definition ) {
			if ( ! in_array( $col, $intel_cols, true ) ) {
				$wpdb->query( "ALTER TABLE `$intel_tbl` ADD COLUMN `$col` $definition" );
			}
		}

		$charset_collate = $wpdb->get_charset_collate();
		$batch_table     = $wpdb->prefix . 'charts_import_batches';
		dbDelta(
			"CREATE TABLE IF NOT EXISTS `{$batch_table}` (
				...
				KEY `run_id` (`run_id`),
				KEY `source_id` (`source_id`),
				KEY `chart_definition_id` (`chart_definition_id`),
