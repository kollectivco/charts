...
class SpotifyConnector extends BaseConnector {
	...
	public function run( $source_id ) {
		return new \WP_Error( 'deprecated', __( 'Spotify page scraping is no longer supported for ranking ingestion. Please use the CSV Import tool.', 'kontentainment-lists' ) );
		...
		if ( ! $source ) {
			return new \WP_Error( 'source_not_found', __( 'Source not found.', 'kontentainment-lists' ) );
		}
		...
		try {
			...
			if ( empty( $rows ) ) {
				throw new \Exception( __( 'No rows extracted from the source.', 'kontentainment-lists' ) );
			}
