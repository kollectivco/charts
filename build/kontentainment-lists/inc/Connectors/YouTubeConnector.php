...
class YouTubeConnector extends BaseConnector {
	...
	public function run( $source_id ) {
		...
		if ( $status_code !== 200 ) {
			$msg = sprintf( __( 'YouTube returned status %d', 'kontentainment-lists' ), $status_code );
		...
		try {
			...
			if ( empty( $rows ) ) {
				$msg = __( 'No rows extracted. Structure might have changed or auth is required.', 'kontentainment-lists' );
				$this->fail_run( $run_id, $msg, $diagnostics );
