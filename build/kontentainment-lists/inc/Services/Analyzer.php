	...
	public function analyze_period( $period_id, $source_id ) {
		...
		if ( $highest_debut ) {
			$this->save_insight( $period_id, $source_id, 'highest_debut', 
				__( 'Highest Debut', 'kontentainment-lists' ),
				sprintf( __( '%s by %s debuts at #%d.', 'kontentainment-lists' ), $highest_debut->track_name, $highest_debut->artist_names, $highest_debut->rank_position ),
				$highest_debut
			);
		}
		...
		if ( $biggest_climber ) {
			$this->save_insight( $period_id, $source_id, 'biggest_climber',
				__( 'Biggest Climber', 'kontentainment-lists' ),
				sprintf( __( '%s jumps %d spots to #%d.', 'kontentainment-lists' ), $biggest_climber->track_name, $max_climb, $biggest_climber->rank_position ),
				$biggest_climber
			);
		}
		...
		if ( $powerhouse && $max_weeks > 5 ) {
			$this->save_insight( $period_id, $source_id, 'longest_streak',
				__( 'List Powerhouse', 'kontentainment-lists' ),
				sprintf( __( '%s has spent %d weeks on the lists.', 'kontentainment-lists' ), $powerhouse->track_name, $max_weeks ),
				$powerhouse
			);
		}
		...
		if ( $top_artist && $count > 1 ) {
			$this->save_insight( $period_id, $source_id, 'most_entries',
				__( 'Dominating the List', 'kontentainment-lists' ),
				sprintf( __( '%s currently has %d tracks on the list.', 'kontentainment-lists' ), $top_artist, $count ),
