...
class YouTubeCsvImporter {
	...
	public function run( $csv_content, array $meta ) {
		...
		if ( empty( $rows ) ) {
			return new \WP_Error( 'empty_csv', __( 'CSV parsed but contained no valid data rows.', 'kontentainment-lists' ) );
		}
		...
		if ( ! $source_id ) {
			return new \WP_Error( 'source_failed', __( 'Could not create or find a YouTube source.', 'kontentainment-lists' ) );
		}
		...
		if ( ! $period_id ) {
			return new \WP_Error( 'period_failed', __( 'Could not create or find a matching period.', 'kontentainment-lists' ) );
		}
		...
		$wpdb->update( $wpdb->prefix . 'charts_import_runs', array(
			...
			'matched_items' => $saved,
			'error_message' => $parse_errors > 0 ? sprintf( __( '%d errors, %d missing titles.', 'kontentainment-lists' ), $parse_errors, $missing_titles ) : ( $missing_titles > 0 ? sprintf( __( '%d missing titles.', 'kontentainment-lists' ), $missing_titles ) : null ),
