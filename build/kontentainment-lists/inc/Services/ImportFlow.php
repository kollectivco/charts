...
class ImportFlow {
	...
	public function run( $source_id ) {
		...
		if ( in_array( $source->source_type, array( 'manual_import', 'metadata_only' ), true ) ) {
			return new \WP_Error( 'manual_only', __( 'This source requires manual file import.', 'kontentainment-lists' ) );
		}
	...
	public function upsert_entry( $source_id, $period_id, $item_type, $item_id, $row, $flat = array() ) {
		...
		$final_score  = isset( $flat['final_score'] ) ? (float) $flat['final_score'] : (float) ( $row['score'] ?? 0 );
		$album_title  = sanitize_text_field( $flat['album_title'] ?? '' );
		$source_platform = sanitize_text_field( $flat['source_platform'] ?? '' );
		$source_provider = sanitize_text_field( $flat['source_provider'] ?? '' );
		$country_code = sanitize_text_field( $flat['country_code'] ?? '' );
		$territory_code = sanitize_text_field( $flat['territory_code'] ?? '' );
		$chart_type   = sanitize_text_field( $flat['chart_type'] ?? '' );
		$source_item_id = sanitize_text_field( $flat['source_item_id'] ?? '' );
		$source_item_key = sanitize_text_field( $flat['source_item_key'] ?? '' );
		$import_batch_id = isset( $flat['import_batch_id'] ) ? intval( $flat['import_batch_id'] ) : 0;
		$score_components_json = $flat['score_components_json'] ?? null;
		...
		$data = array(
			...
			'movement_value'   => isset( $row['movement_value'] ) ? intval( $row['movement_value'] ) : 0,
			'movement_direction' => sanitize_text_field( $row['movement_direction'] ?? '' ),
			'is_new_entry'     => isset( $row['is_new_entry'] ) ? intval( (bool) $row['is_new_entry'] ) : 0,
			'is_reentry'       => isset( $row['is_reentry'] ) ? intval( (bool) $row['is_reentry'] ) : 0,
			'streams'          => $streams,
			...
			'source_item_key'  => $source_item_key ?: null,
			'import_batch_id'  => $import_batch_id ?: null,
			'score_components_json' => $score_components_json,
			'source_url'       => $source_url ?: null,
			'raw_payload_json' => isset( $row['raw_payload_json'] ) ? $row['raw_payload_json'] : wp_json_encode( $row ),
