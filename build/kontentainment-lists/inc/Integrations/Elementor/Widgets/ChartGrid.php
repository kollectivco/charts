...
/**
 * Elementor Widget: List Grid
 */
class ChartGrid extends Widget_Base {

	public function get_name() { return 'charts_grid'; }
	public function get_title() { return __( 'Intelligence Grid', 'kontentainment-lists' ); }
	public function get_icon() { return 'eicon-apps'; }
	public function get_categories() { return [ 'kontentainment-lists' ]; }

	protected function register_controls() {
		$this->start_controls_section( 'section_content', [ 'label' => __( 'Query Configuration', 'kontentainment-lists' ) ] );

		$this->add_control( 'chart_type', [
			'label' => __( 'List Type', 'kontentainment-lists' ),
		...
		$this->add_control( 'limit', [
			'label' => __( 'Limit', 'kontentainment-lists' ),
			'type' => Controls_Manager::NUMBER,
			'default' => 6
		] );

		$this->add_control( 'columns', [
			'label' => __( 'Columns', 'kontentainment-lists' ),
			'type' => Controls_Manager::SELECT,
			'options' => [
				'12' => 'Full Width (1)',
				'6'  => 'Half (2)',
	...
	protected function render() {
		...
		if ( empty( $definitions ) ) {
			echo 'No lists found matching criteria.';
			return;
		}
...
?>
		<div class="kc-card" style="grid-column: span <?php echo $cols; ?>; padding: 0;">
			...
			<div style="padding: 16px 24px; border-top: 1px solid var(--k-border); margin-top: auto;">
				<a href="<?php echo home_url('/lists/' . $def->slug . '/'); ?>" style="font-size: 12px; font-weight: 800; text-decoration: none; color: var(--k-accent);">
					VIEW FULL LIST &rarr;
				</a>
			</div>
		</div>
