...
class YouTubeCsvParser {
	...
	public function parse( $csv_content ) {
		...
		if ( empty( $csv_content ) ) {
			return new \WP_Error( 'empty_csv', __( 'The CSV file is empty.', 'kontentainment-lists' ) );
		}
		...
		if ( ! $headers ) {
			return new \WP_Error( 'no_header', __( 'Could not detect a header row in the CSV.', 'kontentainment-lists' ) );
		}
		...
		foreach ( $headers as $i => $h ) {
			if ( isset( $alias_reverse[ $h ] ) ) {
				...
				$this->warnings[] = sprintf( __( 'Header "%s" is not mapped — will be stored in raw data only.', 'kontentainment-lists' ), $h );
			}
		}
		...
		if ( ! in_array( 'item_title', $mapped_canonicals, true ) && 
			...
			return new \WP_Error( 'bad_headers',
				sprintf( __( 'Could not map required columns (title, ID, or URL). Found headers: %s', 'kontentainment-lists' ), implode( ', ', $headers ) )
			);
		}
		...
		if ( empty( $rows ) ) {
			return new \WP_Error( 'no_rows', __( 'CSV parsed but contained no valid data rows.', 'kontentainment-lists' ) );
		}
	...
	private function normalize_row( $raw, $line_num ) {
		...
		if ( empty( $yt_id ) && empty( $url ) ) {
			$this->warnings[] = sprintf( 
				__( 'Line %d: No YouTube ID or URL found. Metadata may be incomplete.', 'kontentainment-lists' ), 
				$line_num 
			);
		}
