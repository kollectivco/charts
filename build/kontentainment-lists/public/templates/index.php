...
<div class="kc-root">
	...
	<section class="kc-hero-section">
		...
		<div class="kc-hero-content kc-container">
			...
			<p style="max-width:640px; color:var(--k-text-dim); margin-top:16px;">Weekly music lists, artist movement, breakout entries, and editorial intelligence built for discovery, traffic, and social sharing.</p>
			<div style="margin-top:24px; display:flex; gap:12px; flex-wrap:wrap;">
				<a href="<?php echo esc_url( $trending_hub_url ); ?>" class="kc-btn-dashboard" style="text-decoration:none;">Open Trending Hub</a>
				<a href="#all-lists" class="kc-btn-dashboard" style="text-decoration:none; background:rgba(255,255,255,0.08);">Browse Weekly Lists</a>
			</div>
		</div>
	</section>
	...
	<section style="padding: 0 0 60px;">
		<div class="kc-container" style="display:grid; grid-template-columns: 1fr 1fr; gap:24px;">
			<div style="background: var(--k-surface); border:1px solid var(--k-border); border-radius:20px; padding:24px;">
				<h3 style="margin-top:0;">Biggest Climbers</h3>
				<?php foreach ( $biggest_climbers as $row ) : ?>
					<div style="display:flex; align-items:center; gap:12px; padding:10px 0; border-top:1px solid rgba(255,255,255,0.05);">
						...
						<div style="font-size:12px; font-weight:800; color:var(--k-accent-green);">UP <?php echo (int) $row->movement_value; ?></div>
					</div>
				<?php endforeach; ?>
			</div>
			<div style="background: var(--k-surface); border:1px solid var(--k-border); border-radius:20px; padding:24px;">
				<h3 style="margin-top:0;">New This Week</h3>
				<?php foreach ( $new_this_week as $row ) : ?>
					<div style="display:flex; align-items:center; gap:12px; padding:10px 0; border-top:1px solid rgba(255,255,255,0.05);">
						...
						<div style="font-size:12px; font-weight:800; color:var(--k-accent);"><?php echo esc_html( $row->trend_type ?: 'NEW' ); ?></div>
					</div>
				<?php endforeach; ?>
			</div>
		</div>
	...
	<section style="padding: 0 0 60px;">
		<div class="kc-container">
			<header class="kc-section-header">
				<div>
					<div class="kc-header-label">FEATURED LISTS</div>
					<h2 class="kc-header-title">Editor-Selected List Destinations</h2>
				</div>
			</header>
					...
					<a href="<?php echo esc_url( home_url( '/lists/' . $featured_list->slug . '/' ) ); ?>" class="kc-chart-card" style="text-decoration:none; color:inherit;">
						<div class="kc-card-hero" style="height:150px;">
							<div style="position:relative; z-index:10;">
								<span class="kc-card-meta">FEATURED LIST</span>
								<h2 class="kc-card-title"><?php echo esc_html( $featured_list->title ); ?></h2>
							</div>
						</div>
						<div class="kc-card-footer">
							<span class="kc-card-date"><?php echo esc_html( strtoupper( $featured_list->frequency ) ); ?> LIST</span>
							<span class="kc-card-cta" style="color:var(--k-accent);">Open &rarr;</span>
						</div>
					</a>
				<?php endforeach; ?>
			</div>
	...
	<section style="padding: 0 0 60px;">
		<div class="kc-container">
			<header class="kc-section-header">
				<div>
					<div class="kc-header-label">FEATURED ARTISTS</div>
					<h2 class="kc-header-title">Editorial Focus Artists</h2>
				</div>
			</header>
				...
				<?php foreach ( $featured_artists as $artist ) : ?>
					<a href="<?php echo esc_url( home_url( '/lists/artist/' . $artist->slug . '/' ) ); ?>" class="kc-artist-card" style="text-decoration:none; color:inherit;">
						...
						<div class="kc-artist-info">
							...
							<span class="kc-artist-genre">FEATURED ARTIST</span>
						</div>
					</a>
				<?php endforeach; ?>
			</div>
		...
		<section class="kc-featured-strip">
			<header class="kc-section-header">
				<div>
					...
					<h2 class="kc-header-title">Top Artists Lists</h2>
				</div>
				<a href="<?php echo home_url('/lists/' . $artists_chart->slug . '/'); ?>" class="kc-header-link">Full List &rarr;</a>
			</header>
		...
		<section class="kc-all-charts" id="all-lists">
			<header class="kc-section-header">
				<div>
					...
					<h2 class="kc-header-title">All Lists</h2>
				</div>
				...
				?>
					<article class="kc-chart-card">
						...
						<div class="kc-card-hero">
							...
							<div style="position: relative; z-index: 10;">
								<span class="kc-card-meta"><?php echo strtoupper($def->frequency); ?> LIST</span>
