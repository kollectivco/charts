...
if ( ! $definition ) {
	echo '<div class="kc-container" style="padding: 120px 0; text-align: center;"><h1>List Not Found</h1></div>';
...
<div class="kc-root">
	<main class="kc-container">
		...
		<nav style="padding: 40px 0 20px; font-size: 11px; font-weight: 850; letter-spacing: 0.1em; color: var(--k-text-muted);">
			<a href="/lists" style="color: inherit; text-decoration: none;">HOME</a> &nbsp; / &nbsp; 
			<a href="/lists" style="color: inherit; text-decoration: none;">LISTS</a> &nbsp; / &nbsp; 
		...
		<section style="display:flex; gap:12px; flex-wrap:wrap; margin: 0 0 28px;">
			<a href="<?php echo esc_url( 'https://twitter.com/intent/tweet?text=' . rawurlencode( $share_caption ) . '&url=' . rawurlencode( $share_url ) ); ?>" target="_blank" rel="noopener" class="kc-btn-dashboard" style="text-decoration:none;">Share on X</a>
			<a href="<?php echo esc_url( 'https://www.facebook.com/sharer/sharer.php?u=' . rawurlencode( $share_url ) ); ?>" target="_blank" rel="noopener" class="kc-btn-dashboard" style="text-decoration:none; background:rgba(255,255,255,0.08);">Share on Facebook</a>
			<button type="button" class="kc-btn-dashboard" data-share-copy="<?php echo esc_attr( $share_caption ); ?>" data-share-label="Copy Caption" style="background:rgba(255,255,255,0.08);">Copy Caption</button>
		</section>

		<section style="background: var(--k-surface); border:1px solid var(--k-border); border-radius:24px; padding:28px; margin: 0 0 34px;">
			<div class="kc-header-label" style="margin-bottom:12px;">EDITORIAL OVERVIEW</div>
		...
		<section class="kc-rank-hero-box">
			...
			<div class="kc-rank-hero-content">
				...
				<div class="kc-hero-badge-row">
						...
						?>
						<span class="kc-hero-move" style="color: var(--k-accent-green);"><?php echo esc_html( $featured_label ); ?> <?php echo $featured->movement_value ?: 0; ?></span>
					</div>
					...
					<div class="kc-hero-stats-row">
						...
						<div class="kc-hero-stat"><strong><?php echo $featured->weeks_on_chart ?: 1; ?></strong> wks on list</div>
			...
			<div class="kc-rankings-panel">
				<div class="kc-table-head">
					...
					<div style="text-align: right;">Peak</div>
						<div style="text-align: right;">Weeks on List</div>
				</div>
				...
				?>
					<a href="<?php echo esc_url($item_url); ?>" class="kc-rank-row charts-track-click" data-page-type="list" data-object-type="<?php echo esc_attr( $row->item_type ); ?>" data-object-id="<?php echo (int) $row->item_id; ?>" data-slug="<?php echo esc_attr( $item_slug ); ?>">
						...
						<div class="kc-move-val" style="color: <?php echo $dir_color; ?>;">
							<?php echo esc_html( $dir_icon ); ?> <?php echo esc_html( $val_move ); ?>
							<?php echo $growth_str; ?>
						</div>
