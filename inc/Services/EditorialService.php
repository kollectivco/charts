...
class EditorialService {
	...
	public function get_list_editorial_content( $definition, $period, array $entries ) {
		...
		$intro = sprintf(
			charts_tr('%1$s is the definitive weekly music list for %2$s, updated with fresh ranking intelligence for the week of %3$s. This week, %4$s leads the field, while momentum shifts across the top positions highlight where audience attention is moving next.'),
			$definition->title,
			$country,
			$week_of,
			$number_one ? $number_one->track_name . ' ' . charts_tr('by') . ' ' . $number_one->artist_names : charts_tr('the latest top-ranked release')
		);

		$highlights = array();
		if ( $number_one ) {
			$highlights[] = sprintf(
				charts_tr('#1 analysis: %1$s by %2$s holds the top spot with a peak rank of #%3$d and %4$d weeks on the list.'),
		...
		if ( $biggest_climber && (int) $biggest_climber->movement_value > 0 ) {
			$highlights[] = sprintf(
				charts_tr('Biggest climber: %1$s by %2$s jumps %3$d places to #%4$d.'),
		...
		if ( ! empty( $new_entries ) ) {
			$highlights[] = sprintf( charts_tr('New entries: %s.'), implode( ', ', $names ) );
		}
	...
	public function get_share_caption( $context ) {
		...
		if ( $context['type'] === 'list' && ! empty( $context['featured'] ) ) {
			return sprintf(
				charts_tr('#1 this week %1$s %2$s by %3$s leads %4$s.'),
				"\xF0\x9F\x94\xA5",
		...
		if ( $context['type'] === 'track' ) {
			return sprintf( charts_tr('Now trending: %s by %s on Kontentainment Lists.'), $context['item']->title, $context['item']->artist_names ?? charts_tr('featured artists') );
		}
 
		if ( $context['type'] === 'artist' ) {
			return sprintf( charts_tr('%s is making major moves across this week\'s music lists.'), $context['artist']->display_name );
		}
 
		return charts_tr('Trending now on Kontentainment Lists.');
	}

	private function get_index_context() {
		return array(
			'type'        => 'index',
			'title'       => charts_tr('Weekly Music Lists, Trending Artists & Track Rankings'),
			'description' => charts_tr('Explore weekly music lists, trending tracks, rising artists, and editorial highlights across the Egyptian and regional music market.'),
	...
	private function get_trending_context() {
		return array(
			'type'        => 'trending',
			'title'       => charts_tr('Trending Music Lists: Rising Tracks, New Entries & Editorial Picks'),
			'description' => charts_tr('Track what is trending now across the biggest music lists, from rising tracks and breakout entries to editor picks and fast-moving records.'),
	...
	private function get_list_context() {
		...
		$title = sprintf( charts_tr('%s This Week (Updated %s)'), $definition->title, charts_tr(ucfirst( $definition->frequency )) );
		$description = sprintf(
			charts_tr('Explore %1$s for the week of %2$s, including the #1 track, biggest climbers, new entries, and movement across this week\'s music list.'),
			$definition->title,
			charts_tr_date(date_i18n( 'F j, Y', strtotime( $period->period_start ) ))
		);
	...
	private function get_artist_context() {
		...
		return array(
			'type'       => 'artist',
			'title'      => sprintf( charts_tr('%s Charts, Rankings & Music List Profile'), $artist->display_name ),
			'description'=> sprintf( charts_tr('Explore rankings, chart appearances, and editorial intelligence for %s.'), $artist->display_name ),
	...
	private function get_item_context() {
		...
		return array(
			'type'       => 'track',
			'title'      => sprintf( charts_tr('%s by %s Rankings, Trends & List History'), $item->title, $artist_name ? $artist_name : charts_tr('featured artists') ),
			'description'=> sprintf( charts_tr('See trend movement, list appearances, and prediction signals for %s.'), $item->title ),
	...
	private function get_share_card_payload( $type, $slug ) {
		...
		if ( in_array( $slug, array( 'homepage', 'trending' ), true ) ) {
			return array(
				'title'    => $slug === 'homepage' ? charts_tr('Weekly Music Lists') : charts_tr('Trending Now'),
				'subtitle' => charts_tr('Editorial intelligence powered by weekly rankings'),
				'badge'    => $slug === 'homepage' ? charts_tr('FEATURED') : charts_tr('TRENDING'),
				'rank'     => '',
			);
		}
		...
		if ( $type === 'list' ) {
			...
			return array(
				...
				'subtitle' => $entry ? $entry->track_name . ' • ' . $entry->artist_names : charts_tr('Updated editorial rankings'),
				'badge'    => $entry ? ( charts_tr( $entry->movement_direction ?: 'LIST' ) ) : charts_tr('LIST'),
		...
		if ( $type === 'track' ) {
			...
			return array(
				'title'    => $item->title,
				'subtitle' => $artist ?: charts_tr('Track intelligence profile'),
				'badge'    => charts_tr('TRENDING'),
				'rank'     => '',
			);
		}
		...
		if ( $type === 'artist' ) {
			...
			return array(
				...
				'subtitle' => charts_tr('Artist intelligence profile'),
				'badge'    => charts_tr('ARTIST'),
				'rank'     => '',
			);
		}
