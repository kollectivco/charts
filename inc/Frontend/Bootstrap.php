...
class Bootstrap {
	...
	public static function init() {
		...
		add_filter( 'language_attributes', array( self::class, 'force_rtl_attributes' ), 999, 2 );
	}

	/**
	 * Force RTL language and direction for the charts pages frontend.
	 */
	public static function force_rtl_attributes( $output, $doctype ) {
		if ( \Charts\Core\StandaloneLayout::is_charts_page() ) {
			return 'lang="arz" dir="rtl"';
		}
		return $output;
	}
		...
		if ( get_query_var( 'charts_page' ) || get_query_var( 'charts_platform' ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'copied'  => charts_tr( 'Copied' ),
					'copy'    => charts_tr( 'Copy Caption' ),
				)
			);
		}
	}
