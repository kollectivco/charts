...
jQuery(document).ready(function($) {
    ...
    /**
     * Auto Translate API Engine
     */
    const $autoBtn = $('#kc-auto-translate-btn');
    if ($autoBtn.length) {
        ...
        const updateProgress = (completed, total) => {
            ...
            $('#kc-auto-translate-status').text(`${completed} / ${total}`);
            $('#kc-auto-translate-bar').css('width', `${percent}%`);
        };

        const processNextChunk = () => {
            if (currentChunk * chunkSize >= untranslatedStrings.length) {
                // Done
                isTranslating = false;
                $autoBtn.html('<span class="dashicons dashicons-saved" style="margin-top:-2px;"></span> Translation Complete');
                window.ChartsToast.show('success', 'All missing strings have been translated and saved!', 'Translation Complete');
                setTimeout(() => location.reload(), 2000);
                return;
            }

            const chunk = untranslatedStrings.slice(currentChunk * chunkSize, (currentChunk + 1) * chunkSize);
            
            $.ajax({
                url: ajaxurl,
                type: 'POST',
                data: {
                    action: 'charts_api_translate_strings',
                    _wpnonce: $('#_wpnonce').val(), // Use the nonce from the form
                    step: 'translate_chunk',
                    strings: chunk
                },
                success: function(res) {
                    if (res.success) {
                        currentChunk++;
                        const completed = Math.min(currentChunk * chunkSize, untranslatedStrings.length);
                        updateProgress(completed, untranslatedStrings.length);
                        
                        // Process next chunk
                        setTimeout(processNextChunk, 500); // 500ms delay to respect rate limits
                    } else {
                        window.ChartsToast.show('error', res.data || 'Failed to translate chunk.', 'Translation Error');
                        isTranslating = false;
                        $autoBtn.html('<span class="dashicons dashicons-admin-site-alt3" style="margin-top:-2px;"></span> Resume Translation').prop('disabled', false);
                    }
                },
                error: function() {
                    window.ChartsToast.show('error', 'Server error during translation.', 'Translation Error');
                    isTranslating = false;
                    $autoBtn.html('<span class="dashicons dashicons-admin-site-alt3" style="margin-top:-2px;"></span> Resume Translation').prop('disabled', false);
                }
            });
        ...
        $autoBtn.on('click', function(e) {
            ...
            $autoBtn.html('<span class="dashicons dashicons-update" style="margin-top:-2px;" class="spin"></span> Initializing...').prop('disabled', true);
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.success) {
                        untranslatedStrings = res.data.strings || [];
                        
                        if (untranslatedStrings.length === 0) {
                            $autoBtn.html('<span class="dashicons dashicons-saved" style="margin-top:-2px;"></span> Everything Translated');
                            window.ChartsToast.show('info', 'There are no untranslated tracks or videos in the database.', 'All Caught Up');
                            return;
                        }

                        // Start UI
                        $('#kc-auto-translate-progress-container').slideDown();
                        updateProgress(0, untranslatedStrings.length);
                        isTranslating = true;
                        currentChunk = 0;
                        
                        $autoBtn.html('<span class="dashicons dashicons-update" style="margin-top:-2px;" class="spin"></span> Translating via API...');
                        processNextChunk();

                    } else {
                        window.ChartsToast.show('error', res.data || 'Failed to fetch strings.', 'Translation Error');
                        $autoBtn.html('<span class="dashicons dashicons-admin-site-alt3" style="margin-top:-2px;"></span> Auto-Translate via API').prop('disabled', false);
                    }
                },
                error: function() {
                    window.ChartsToast.show('error', 'Server error checking for strings.', 'Translation Error');
                    $autoBtn.html('<span class="dashicons dashicons-admin-site-alt3" style="margin-top:-2px;"></span> Auto-Translate via API').prop('disabled', false);
                }
            });
        });
