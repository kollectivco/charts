...
<div class="cm-app-shell">
    ...
    <main class="cm-content">
        <!-- Hero -->
        <section class="cm-section" style="padding: 24px 20px;">
            <div style="display: flex; gap: 20px; align-items: center;">
                <img src="<?php echo esc_url($item->cover_image ?: CHARTS_URL . 'public/assets/img/placeholder.png'); ?>" style="width: 100px; height: 100px; border-radius: 12px; object-fit: cover; box-shadow: var(--cm-shadow-sm);">
                <div style="flex: 1;">
        ...
        <section class="cm-section" style="padding: 0 20px 40px;">
            <h3 style="font-size: 11px; font-weight: 900; text-transform: uppercase; color: var(--cm-text-muted); margin-bottom: 16px;">Chart History</h3>
            
            <div style="display: flex; flex-direction: column; gap: 12px;">
                <?php if ( empty($appearances) ) : ?>
                    <div class="cm-card" style="padding: 20px; text-align: center; color: var(--cm-text-muted); font-size: 13px;">No chart history recorded yet.</div>
                <?php else : ?>
                    <?php foreach ( $appearances as $app ) : ?>
                        <div class="cm-card" style="padding: 16px; display: flex; align-items: center; justify-content: space-between;">
                            <div style="display: flex; align-items: center; gap: 12px;">
                                <div style="width: 32px; height: 32px; background: <?php echo !empty($app->accent_color) ? $app->accent_color : 'var(--cm-primary)'; ?>; border-radius: 8px; display: flex; align-items: center; justify-content: center; color:#fff;">
                                ...
                                <div>
                                    ...
                                    <span style="font-size: 10px; color: var(--cm-text-muted);"><?php echo date('M j, Y', strtotime($app->period_start)); ?></span>
                                </div>
                            </div>
                            <div style="text-align: right;">
                                <div style="font-size: 20px; font-weight: 950; color: var(--cm-text);">#<?php echo $app->rank_position; ?></div>
                                <?php if ( ! empty($app->peak_rank) ) : ?>
                                    <div style="font-size: 9px; font-weight: 800; color: var(--cm-text-muted);">PEAK #<?php echo $app->peak_rank; ?></div>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
